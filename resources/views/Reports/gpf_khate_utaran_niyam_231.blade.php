...
<body class="nav-md">
  @if(count($rqo_result))
    @foreach($rqo_result AS $rqo)
      ...
      <div class="col-md-12 col-sm-12 "><div class="x_panel"><div class="x_content"><div class="row">      <div class="col-md-2"><img src="{{URL('asset/images/zp-nashik-bharti.jpg') }}" width="120"></img></div><div class="col-md-8" style="text-align: center;"><h2> <b>जिल्हा परिषद नाशिक </b></h2><h2> <b>भविष्य निर्वाह निधी खाते उतारा (नियम २३१)  </b></h2><h2> <b>सन ({{digitChange(session()->get('financial_year'))}})</b> </h2></div><div class="col-md-2"><h2>{{digitChange(date('d/m/Y h:i:s'))}}</h2></div><div class="col-sm-12"><div class="card-box "><div class="card-box "><div class="x_title"></div><div class="col-md-2 lg-4"> <label> खाते क्रमांक :- {{$rqo->inital_letter}}{{digitChange($rqo->gpf_number)}} </label></div><div class="col-md-7" style="text-align: center;"> <label> कर्मचाऱ्याचे नाव :- {{$rqo->employee_name}}</label></div><div class="col-md-3 lg-4" style="text-align:end;"><label>पदनाम :- {{$rqo->designation_name}}</label></div><div class="col-md-6"><label>तालुका / मुख्यालयाचे नाव :- {{$rqo->taluka_name}}</label></div><div class="col-md-6 " style="text-align: end;"><label>विभाग /कार्यालयाचे नाव :- {{$rqo->department_name}}</label></div></div>@php $otherInstall = DB::table('master_vetan_ayog_received AS va')->select('va.instalment','va.DiffAmt','va.Interest')->where(['va.GPFNo' =>$rqo->gpf_number,'va.Year'=>session()->get('from_year'),'va.pay_number'=>6])->get();
                      ...
                      @if($tcount)
                        @for($j=0;$j < $tcount;$j++)
                          ...
                          @endphp
                          @endfor
                      @endif<table style="width:100%"><tr><th>माहे </th><th>मासिक वर्गणी </th><th> कर्ज वसुली हप्ता</th><th>इतर जमा </th><th>एकूण जमा</th><th> कर्ज रक्कम</th><th>शेरा </th></tr><thead></thead><tbody>@php
                        $opening_balance = $rqo->opening_balance;
                        $intrest_rate = 8;
                        $total_gpf = [];
                        $total_ins_amt_one = $total_ins_amt;
                        ...
                        @foreach($month_name AS $key => $month)
                          ...
                          @endphp<tr><td>{{$month->month_name}} - {{digitChange(($key < 9)?session()->get('from_year'):session()->get('to_year'))}}</td><td class="amounttext">{{digitChange($rqo->$monthly_contrubition)}}</td><td class="amounttext">{{digitChange($rqo->$loan_installment)}}</td><td class="amounttext">{{digitChange($total_ins_amt_one)}}</td><td class="amounttext">{{digitChange((($rqo->$monthly_contrubition+$rqo->$loan_installment) - $rqo->$loan_amonut))}}</td><td class="amounttext">{{digitChange($rqo->$loan_amonut)}}</td><td></td></tr>@php
                            $total_ins_amt_one = 0;
                        ...
                        @endif<tr><td>वर्षातील जमा रकमेवर व्याज </td><td class="amounttext">{{digitChange($total_intrest)}}</td><td colspan="3"> ६ वा वेतन हप्ता रक्कम एकूण </td><td colspan="3" class="amounttext">{{digitChange($total_ins_amt)}}</td>
                          ...
                          <tr><td> एकूण जमा रक्कम </td><td class="amounttext">{{digitChange(($rqo->opening_balance+$total_one+$total_two+$total_ins_amt+$total_ins_interest+$total_intrest))}}</td><td colspan="3">व्याजदर @if(count($roi_result))
                            @foreach($roi_result AS $rowroi)
                            {{getMonthName($rowroi->to_month)}}
                            {{$rowroi->percent}}
                            @endforeach
                            {{getMonthName(3)}}
                            @endif</td><td colspan="3"></td></tr><tr><td>वर्षातील काढून घेतलेल्या रकमा </td><td class="amounttext">{{digitChange($total_four)}}</td></tr></tbody></table><div class="col-md-12 mt-3"><label> दिनांक ३१ मार्च {{digitChange(session()->get('to_year'))}} - अखेर शिल्लक रक्कम रु . {{digitChange(($rqo->opening_balance+
                              $total_one+$total_two+$total_ins_amt+$total_ins_interest+$total_intrest)-$total_four)}} </label></div><div class="col-md-12">  <label> अक्षरी रु .{{convertToIndianCurrency(($rqo->opening_balance+
                                $total_one+$total_two+$total_ins_amt+$total_ins_interest+$total_intrest)-$total_four)}} </label></div><div class="col-md-12 mt-2"><h4 class="text-center mt-3" style="border: 1px solid black;padding-block: 8px;"> <b>७ वा वेतन आयोग फरक जमा </b></h4><table style="width:100%"><thead><tr><th>हप्ता नं </th><th> महिना / वर्ष</th><th> व्याज देय दिनांक</th><th> रक्कम </th><th> वर्ष १ व्याज </th><th> वर्ष २ व्याज </th><th> एकूण व्याज </th><th>वार्षिक जमा</th><th>अनाहरणीय </th></tr></thead>
                              @php
                              ...
                              <tbody>
                                ...
                                <tr>
                                  ...
                                  <td colspan="4" class="amounttext">
                                    {{digitChange(($rqo->opening_balance+$total_one+$total_two+$total_ins_amt+
                                      $total_ins_interest+$total_intrest)-$total_four)}} + {{digitChange($totalRecivedDiff)}} = {{digitChange(($rqo->opening_balance+$total_one+$total_two+$total_ins_amt+
                                      $total_ins_interest+$total_intrest+$totalRecivedDiff)-$total_four)}}
                                  </td>
                                </tr>
                                <tr>
                                  <td colspan="5"></td>
                                  <td colspan="4">
                                    {{convertToIndianCurrency(($rqo->opening_balance+$total_one+$total_two+$total_ins_amt+$total_ins_interest+$total_intrest+$totalRecivedDiff)-$total_four)}}
                                  </td>
                                </tr>
                              </tbody>
                              </table>
                              <br></div><div class="col-md-12 mt-12" style="text-align:end;"><br><br><div class="row" style="text-align:center;"><div class="col-md-4 mt-4"><h2> </h2><h2> </h2></div><div class="col-md-4 mt-4"><h2>  </h2><h2></h2></div><div class="col-md-4 mt-4"><h2> उपमुख्य लेखा वा वित्त अधिकारी</h2><h2> जिल्हा परिषद नाशिक </h2></div></div><div class="row"><h2><b> टीप :- </b> वरील हिशोबामध्ये काही तफावत आढळल्यास १५ दिवसांच्या आत नाशिक जिल्हा  परिषद वित्त विभागाशी आपल्या खातेप्रमुखामार्फत संपर्क साधावा .</h2></div></div></div></div></div></div></div></div><p>&nbsp;</p>
@endif
@endforeach
@endif
</body></html>
