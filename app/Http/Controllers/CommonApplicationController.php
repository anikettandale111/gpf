...
class CommonApplicationController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('Application/commonform',compact('collapse_div','active_menu','page_name','reasons'));
    }
    ...
    public function getuserdetailsbygpfno(Request $request){
      // return ganrate::where('gpf_no',$request->input_id)->first();
        $lang = app()->getLocale();
        return DB::table('ganrate_new_number as gn')
                ...
                ->join('classifications as cl','cl.id','=','gn.classification')
                ->join('taluka as tl','tl.id','=','gn.taluka_id')
                ->select('gn.employee_id','gn.gpf_no','gn.employee_name','gn.joining_date','gn.retirement_date',
                        'gn.total_service','gn.bank_account_no','gn.branch_location','gn.ifsc_code','gn.bank_id',
                        'gn.department_id','gn.designation_id','gn.classification','gn.id','bk.bank_name_'.$lang.' as bank_name','tl.taluka_name_'.$lang.' as taluka_name','dg.designation_name_'.$lang.' as designation_name','dp.department_name_'.$lang.' as department_name','cl.classification_name_'.$lang.' as classification_name','cl.inital_letter',)
                ->where('gn.gpf_no',$request->input_id)
                ->get();
    }
}
