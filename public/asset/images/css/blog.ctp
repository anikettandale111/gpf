<?php if(isset($type) && ($type == 'article')): ?>
	...
	<meta property="og:url"  content="<?= webURL.'blog/?article='.$article->slug;?>" />
	...
	<link rel="shortcut icon" href="<?= webURL ?>image/config/admni_logo_image574.png" type="image/png">
    <link rel="icon" href="<?= webURL ?>image/config/admni_logo_image574.png" type="image/png">
	...
	<div id="primaryR1" class="pad-hero" data-grid="container" data-region-key="primaryr1">
			...
			<div id="coreui-hero-7enwkdp">
							...
							<div class="m-hero blog">
									...
									<picture>
									<source srcset="<?= imageURL.'blog/banner/500/'.$blogBanner['thumbnail'] ?>" media="(max-width:539px)">
											<source srcset="<?= imageURL.'blog/banner/700/'.$blogBanner['thumbnail'] ?>" media="(min-width:540px) and (max-width:767px)">
											<source srcset="<?= imageURL.'blog/banner/1000/'.$blogBanner['thumbnail'] ?>" media="(min-width:768px) and (max-width:1083px)">
											<source srcset="<?= imageURL.'blog/banner/1200/'.$blogBanner['thumbnail'] ?>" media="(min-width:1084px) and (max-width:1399px)">
											<source srcset="<?= imageURL.'blog/banner/1600/'.$blogBanner['thumbnail'] ?>">
							...
							<div>
									<div>
						<?php $data = get_object_vars(json_decode($blogBanner['English'])); ?>
													...
													<?php else: ?>
															<div class="post-gallery-slide slick-slide image">
															    <a href="javascript:void(0)" imageanchor="1">
															        <picture>
																		<source media="(min-width: 1200px)" srcset="<?= imageURL.'blog/'.$ivalue['image'] ?>">
																		<source media="(max-width: 1200px)" srcset="<?= imageURL.'blog/900/'.$ivalue['image'] ?>">
																		<source media="(max-width: 768px)" srcset="<?= imageURL.'blog/650/'.$ivalue['image'] ?>">
																		<img class="img-responsive depart-img" src="<?= imageURL.'blog/400/'.$ivalue['image'] ?>" alt="<?= $ivalue['alt'] ?>" title="<?= $ivalue['title'] ?>" media-simple="true">		
																	</picture>	
															    </a>
															  </div>
													<?php endif; ?>
								...
								<div class="bottom-blog">
									...
									<section class="rating-widget">
										<div class='rating-stars text-center'>
											<?php if(isset($cookie) && (!is_array($cookie))): ?>
													<ul id='stars-old'>				
														<?php for($i = 1; $i <= 5; ++$i){ ?>
															<?php if($i <= $cookie): ?>
																	<li class='star-old hover' title='Poor'>
															<?php else: ?>
																	<li class='star-old' title='Poor'>
															<?php endif; ?>
									...
									<ul class="group-share pull-right">
										...
										<li><a href="https://www.facebook.com/sharer.php?u=<?= $articleurl ?>" title="<?= urlencode($article->title) ?>" target='_blank'><i class='fa fa-facebook'></i></a></li>
										<li><a href="https://twitter.com/share?url=<?= $articleurl ?>&text=<?= urlencode($article->title) ?>" target='_blank'><i class='fa fa-twitter'></i></a></li>
										<li class="linkedin"> 
											<a href="https://www.linkedin.com/shareArticle?mini=true&amp;url=<?= webURL.'article/index/'.$article->slug ?>&amp;title=<?= $article->title ?>&amp;summary=<?= substr(strip_tags($article->description), 0, 100) ?>&amp;source=" title="Linkedin" target="_blank" rel="noopener noreferrer"><i class="fa fa-linkedin" aria-hidden="true"></i></a>
										</li>
									</ul>
					...
					<div class="col col-xs-12 col-sm-6 col-md-4 col-lg-4 col1 blog_wrapper blogItem">
						...
						<div class="promo column unit6">
								<span class="promo-image column unit4 noHighContrastAdjust">
									...
									<?php if($video): ?>
										<span class="promo-image column unit4 noHighContrastAdjust youtube"><a href="<?= $articleurl ?>" videoanchor="1" style="display:block;">
									...
									<?php else: ?>
										<span class="promo-image column unit4 noHighContrastAdjust image">
								...
								<div class="promo-content column blog_content">
									<a href="<?= $articleurl ?>">
	...
	<script>
			/* rating script start */
				...
				$(document).ready(function(){
					...
					if ( $('.zippy').hasClass('toggle-open') )
					{
					$('.zippy').removeClass('toggle-open');
					$('.zippy').html('&#x25BA;&#160;');
					}
					...
					if ( $(this).parent().hasClass('expanded') )
					{
						...
						$(this).find(".zippy").removeClass('toggle-open').html('&#x25BA;&#160;');
					}else{
						$(this).parent().removeClass('collapsed');
						$(this).parent().addClass('expanded');
						$(this).find(".zippy").addClass('toggle-open').html('&#9660;&#160;');
					}
					});
				  ...
				  $('#stars li').on('mouseover', function(){
				    ...
				    $(this).parent().children('li.star-half').each(function(e){
				      if (e < onStar) {
				  ...
				  $('#stars li').on('click', function(){
				    ...
				    $.ajax({
			          ...
			          dataType: "json",
			          url: url,
			          data: {id:article_id, rating: rating, '_csrfToken' : '<?= $this->request->param("_csrfToken") ?>'},  
			          success: function(message){
			            //location.reload();
			            if(message['status'] == 'success'){
			    					responseMessage(msg);
										$(".success-box").show("slow")
										setTimeout(function() {location.reload();}, 3000);
										setTimeout(function(){ $(".success-box").hide("slow") }, 2800);
			            }
			            if(message['status'] == 'already-rated'){
			            		alert("Already rated"); location.reload();
			            }
			          }
		      });
