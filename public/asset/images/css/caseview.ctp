...
<meta property="og:title" content="Urrzaa">
...
<link rel="stylesheet" href="<?= webURL ?>css/case_lightbox.css">
<!-- UHF Styles -->
      ...
      <div class="m-hero ">
        <div>
          <section role="presentation" class="m-hero-item f-medium f-x-left f-y-center context-game theme-dark f-mask-left f-short">
            ...
            <picture>
              <source srcset="<?= webURL.'image/caseview/banner/500/'.$image ?>" media="(max-width:539px)">
              <source srcset="<?= webURL.'image/caseview/banner/700/'.$image ?>" media="(min-width:540px) and (max-width:767px)">
              <source srcset="<?= webURL.'image/caseview/banner/1000/'.$image ?>" media="(min-width:768px) and (max-width:1083px)">
              <source srcset="<?= webURL.'image/caseview/banner/1200/'.$image ?>" media="(min-width:1084px) and (max-width:1399px)">
              <source srcset="<?= webURL.'image/caseview/banner/1600/'.$image ?>">
              <img class="lazypreload" alt="<?= $banner['medias'][0]['alt'] ?>" title="<?= $banner['medias'][0]['title'] ?>" src="<?= webURL.'image/caseview/banner/500/'.$image ?>">
            </picture>
            <div>
              <div>
	...
	<div class="content standardWidth ">
      <div class="row colDef2">
        <?php foreach ($caseviews as $key => $caseview) { $caseviewImage = ''; ?>
            <div class="col col-sm-6 col1 noTheme horizontalAlignCenter verticalAlignTop paddingHorizontal1 paddingVertical1 cd-section">
              <div class="colContent">
                <div class="promo column unit6">
                    <?php
					  $caseviewImage = $caseview['thumbnail'];
                    ?>
          				<div class="promo-image column unit4 noHighContrastAdjust" style="background-image: url(<?= webURL.'image/caseview/'.$caseviewImage ?>) !important;">
                    
                    <picture>
          						<source srcset="<?= webURL.'image/caseview/1x/'.$caseviewImage ?>" media="(max-width:599px)">
                      <source srcset="<?= webURL.'image/caseview/2x/'.$caseviewImage ?>" media="(min-width:600px)">
                      <source srcset="<?= webURL.'image/caseview/3x/'.$caseviewImage ?>" media="(min-width:600px)">
          						<img class="lazypreload" alt="<?= $caseview['thumbnail_alt'] ?>" title="<?= $caseview['thumbnail_alt'] ?>" src="<?= webURL.'image/caseview/'.$caseviewImage ?>">
          					</picture>
          				</div>
          				<div class="promo-content column unit8">
                    <?php $data = get_object_vars(json_decode($caseview['English'])); ?>
                    <?php if($data['link_text_a'] && ($data['link_text_a_appearence'] == 1)): ?>
                      <h2  class="fontSizeH2 <?= $alignment[$data["link_text_a_alignment"]] ?>">
                        <?= $data['link_text_a'] ?>
                      </h2>
                    <?php endif; ?>
                    <?php if($data['link_text_b'] && ($data['link_text_b_appearence'] == 1)): ?>
                      <h4 class="fontSizeH4 <?= $alignment[$data["link_text_b_alignment"]] ?>">                    
                      <?= $data['link_text_b'] ?>
                      </h4>
                    <?php endif; ?>
          					<?php if($data['link_text_c'] && ($data['link_text_c_appearence'] == 1)): ?>
                      <p class="<?= $alignment[$data["link_text_c_alignment"]] ?>"><?= $data['link_text_c'] ?></p>
                    <?php endif; ?>
          						<div class="cd-modal-action">
          							<a class="btn promo-link" title="Explore Gursha" href="<?= webURL.'home/caseview_detail/'.$caseview['id'] ?>" data-type="modal-trigger"><?= $data['link_text'] ?></a>
          							<span class="cd-modal-bg"></span>
          						</div>
          				</div>
          				<div class="cd-modal">
          					<div class="cd-modal-content full-width">
...
<script src="<?= webURL ?>js/velocity.min.js"></script>
<script src="<?= webURL ?>js/case_lightbox.js"></script> <!-- Resource jQuery -->
