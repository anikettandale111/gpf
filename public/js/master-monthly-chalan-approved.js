...
$(document).ready(function(){
  ...
  $('.checkallapproved').change(function(){
    if(this.checked) {
      $('.check_app').prop("checked",true);
      $('.check_app').each(function() {
        var rid = $(this).val();
        approvedRowsId.push(rid);
      });
    }else{
      $('.check_app').prop("checked",false);
      approvedRowsId = [];
    }
  });
      ...
      if (res.amt != null) {
        ...
        if ((res.chalan).length) {
          ...
          $(res.chalan).each(function(key, val) {
            if(val.is_active){
              btn = '<label class="btn btn-success"> मंजूर केले आहे </label>'
            }else{
              btn = '<input type="checkbox" class="check_app app_row_'+val.emc_id+'" name="approve_rowid[]" value="'+val.emc_id+'">';
            }
            str += '<tr><td>' + i + '</td><td>' + val.emc_year + '</td><td>' + (val.month_name + val.challan_number) + '</td><td>' + val.taluka_name + '</td><td>' + val.gpf_number + '</td><td>' + val.employee_name + '</td><td>' + val.monthly_contrubition + '</td><td>' + val.loan_installment + '</td><td>' + val.monthly_other + '</td><td>' + (parseInt(val.monthly_contrubition) + parseInt(val.loan_installment) + parseInt(val.monthly_other)) + '</td><td>' + val.name + '</td><td>'+btn+'</td></tr>';
            i++;
          });
        }
...
function makeApproved(){
  ...
  if(approvedRowsId.length ){
    swal({
      title: "Approved ?",
      text: "Once Approved you can not change it again!",
      type: "warning",
      showCancelButton: !0,
      ...
      if (e.value === true) {
        $.ajax({
          type: 'POST',
          url: "monthlyEntryApproved/",
          data: {_token: CSRF_TOKEN,id_data:approvedRowsId},
          success: function (results) {
            swal(results.status,results.message);
          }
        });
    ...
    swal('Error','Please Select Atleast One Checkbox');
    return false;
  }
}
